...
def preprocess_json(raw_json):
    """
    Escapes Unescaped Backslashes for Windows-style Paths provided in 'test.json' 
    """
...
def load_config(): 
    """Loads and validates configuration from test.json."""
    try:
        ...
        try:
            ...
            try:
                ...
                print("[ERROR] Invalid JSON format in 'test.json'! Please correct it and Try Again.")
                sys.exit(1)
        ...
        for field in required_fields:
            if field not in config:
                raise ValueError(f"Missing Required Config Section: {field} inside 'test.json'")
            
        # Sets Current Directory for Download if 'download_settings' not specified
        if "download_settings" not in config:
            print("\n[Warning]: 'download_settings' not set in 'test.json'. Downloading in the Current Directory..")
...
# Fetching Information from 'test.json'
...
# Retrives Download Path from 'test.json'
...
failed_downloads_file = download_settings.get("failed_downloads_file", "failed_downloads.txt")
retry_wait_seconds = int(download_settings.get("retry_wait_seconds", 30) or 30)
...
# Validates Fields of 'download_settings' of 'test.json'
...
if invalid_fields:
    ...
    print("\nPlease Correct these in your 'test.json' and Try Again.\n")
    sys.exit(1)
...
def get_failed_downloads_path():
    if os.path.isabs(failed_downloads_file):
        return failed_downloads_file
    return os.path.join(os.getcwd(), failed_downloads_file)

def build_failed_entry(identifier, record_id, prod_date):
    return {
...
def read_failed_downloads():
    file_path = get_failed_downloads_path()
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            identifier = parts[0]
            record_id = parts[1]
            prod_date = parts[2] if len(parts) > 2 else ""
            failed_entries.append(build_failed_entry(identifier, record_id, prod_date))
    return failed_entries

def write_failed_downloads(failed_entries):
    file_path = get_failed_downloads_path()
    ensure_parent_dir(file_path)

    with open(file_path, "w", encoding="utf-8") as file:
        for entry in failed_entries:
            file.write(f"{entry['identifier']}\t{entry['record_id']}\t{entry['prod_date']}\n")

def append_failed_download(entry):
    failed_entries = read_failed_downloads()
    if any(existing["identifier"] == entry["identifier"] for existing in failed_entries):
        return
    failed_entries.append(entry)
    write_failed_downloads(failed_entries)

def remove_failed_download(identifier):
    failed_entries = [entry for entry in read_failed_downloads() if entry["identifier"] != identifier]
    write_failed_downloads(failed_entries)
...
def search_results():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"\n[ERROR] Error Fetching Data from Search Endpoint. Please enter correct 'search_parameters' in your 'test.json' and try again.\n\nStatus Code: {res.status_code}\nError Message: {error_message}\n")
            if generate_logs:
...
def fetch_and_download_data(total_files, access_token, refresh_token):
    ...
    try:
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for item in list['entries']:
                        ...
                        result = download_data(access_token, record_id, identifier, prod_date, counter, total_files)
                        ...
                        if status == "downloaded" and file_path and os.path.exists(file_path):
                            download_count += 1
                            remove_failed_download(identifier)
                        elif status == "exists":
                            skip_count += 1
                            remove_failed_download(identifier)
                        elif status == "failed":
                            append_failed_download(build_failed_entry(identifier, record_id, prod_date))
                        elif status == "Permission Denied":
                            append_failed_download(build_failed_entry(identifier, record_id, prod_date))
                            raise PermissionError
                        elif status not in ("NOT_RELEASED", "Invalid/Expired Token"):
                            append_failed_download(build_failed_entry(identifier, record_id, prod_date))
        ...
        if counter == (total_files + 1):
            return True, download_count, skip_count, access_token, refresh_token
        else:
            return False, 0, 0, access_token, refresh_token
...
def retry_failed_downloads(access_token, refresh_token):
    attempt = 1

    while True:
        failed_entries = read_failed_downloads()
        if not failed_entries:
            print(f"\n{GREEN}No failed downloads pending. Retry queue is empty.{RESET}")
            return access_token, refresh_token

        print(f"\nRetry attempt {attempt}: {len(failed_entries)} file(s) still pending in {get_failed_downloads_path()}")
        remaining_entries = []
        recovered_count = 0
        ...
        for index, entry in enumerate(failed_entries, start=1):
            ...
            prod_date = entry["prod_date"] or None

            result = download_data(access_token, record_id, identifier, prod_date, index, len(failed_entries))
            status = result["status"]
            file_path = result.get("file_path")
            ...
            if status == "NOT_RELEASED":
                print(f"\n[WARNING] {identifier} is not released on MOSDAC yet. Keeping it in the retry list.")
                remaining_entries.append(entry)
                continue

            if status == "Permission Denied":
                remaining_entries.append(entry)
                write_failed_downloads(remaining_entries + failed_entries[index:])
                raise PermissionError

            remaining_entries.append(entry)

        write_failed_downloads(remaining_entries)

        if not remaining_entries:
            print(f"\n{GREEN}All failed downloads were recovered. Retry queue is now empty.{RESET}")
            return access_token, refresh_token

        if recovered_count == 0:
            print(f"\n[INFO] No files were recovered in retry attempt {attempt}. Waiting {retry_wait_seconds} seconds before retrying again.")
        else:
            print(f"\n[INFO] Recovered {recovered_count} file(s). {len(remaining_entries)} file(s) still pending.")
...
def get_user_input():
    try:
        if skip_user_input:
            print(f"\n{GREEN}'skip_user_input' Option Enabled in 'test.json'. Proceeding with Download...{RESET}")
            return "yes"
        ...
        for attempt, delay in enumerate(RETRY_DELAYS + [None]):
            try:
                ...
                if os.path.exists(file_path):
                    ...
                    return {"status": "exists", "file_path": file_path}
                ...
                if response.status_code == 400:
                    ...
                    return {"status": "failed", "file_path": None}
                ...
                if response.status_code == 401:
                    ...
                    if error_data.get("code") == "NO_ACCESS_TOKEN":
                        ...
                        return {"status": "Invalid/Expired Token", "file_path": None}
                    
                if response.status_code == 404:
                    error_data = response.json()
                    if error_data.get("code") == "NOT_RELEASED":
                        return {"status": error_data.get("code"), "file_path": None}
                ...
                with open(tmp_file_path, "wb") as file:
                    if HAS_TQDM:
                        try:
                            ...
                            return {"status": "Permission Denied", "file_path": None}
                        except Exception as e:
                ...
                return {"status": "downloaded", "file_path": file_path}
...
def main():
    ...
    download_complete, download_count, skip_count, access_token, refresh_token = fetch_and_download_data(total_files, access_token, refresh_token)
    access_token, refresh_token = retry_failed_downloads(access_token, refresh_token)
    ...
    if download_complete:
        ...
        print(f"\n[INFO] Initial pass did not complete cleanly, but the failed download retry loop has been started using {get_failed_downloads_path()}.")
