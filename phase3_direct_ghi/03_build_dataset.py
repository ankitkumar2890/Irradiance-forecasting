"""
03_build_dataset.py — Temporal split → sliding window .npy + GluonTS Arrow.

Phase 3: Direct GHI forecasting.
...
Key difference from Phase 2:
  - y_future = GHI (w_ghr) in W/m², scaled by GHI_SCALE_FACTOR
  - All cluster stations included in each split
"""
import sys
import argparse
import numpy as np
...
def build_windows(df, split_name):
    ...
    The target is GHI (w_ghr), scaled by GHI_SCALE_FACTOR for training stability.
    """
    ...
    for station_id, station_df in df.groupby("station_id", sort=True):
        ...
        for i in range(len(station_df) - PAST_HOURS - FUTURE_HOURS + 1):
            ...
            # Target: GHI in W/m², scaled down for training stability
            y_future.append(future["w_ghr"].values / GHI_SCALE_FACTOR)
...
def build_arrow(df, split_name):
    ...
    Target is GHI (w_ghr) scaled by GHI_SCALE_FACTOR.
    """
    ...
    for station_id, station_df in df.groupby("station_id", sort=True):
        ...
        if station_df.empty:
            continue
        # Target: GHI scaled
        target = (station_df["w_ghr"].values / GHI_SCALE_FACTOR).astype(np.float32)
...
def main():
    ...
    if "station_id" not in df.columns:
        raise ValueError("Processed CSV must contain a station_id column for cluster training.")
