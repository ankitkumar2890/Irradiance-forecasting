"""Evaluate the custom Moirai 1.1 seq2seq GHI forecasts."""
...
ONE_WEEK_DAYS = 7
RANDOM_WEEK_SEED = 42
DEFAULT_PREDICTION_FILE = RESULTS_DIR / f"moirai1_seq2seq_{CLUSTER_NAME}_val_predictions.csv"
DEFAULT_OUTPUT_PREFIX = f"moirai1_seq2seq_{CLUSTER_NAME}"
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, label: str = "") -> dict[str, float | int | str]:
...
def summarize_subset(df: pd.DataFrame, label: str, mape_threshold: float = DEFAULT_GHI_FILTER_WM2):
    if df.empty:
        return None, None
...
def slice_window(df: pd.DataFrame, start: pd.Timestamp, days: int = ONE_WEEK_DAYS) -> pd.DataFrame:
...
def choose_random_later_week_start(
    ...
    days: int = ONE_WEEK_DAYS,
    seed: int = RANDOM_WEEK_SEED,
) -> pd.Timestamp:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ghi-filter-wm2", type=float, default=DEFAULT_GHI_FILTER_WM2)
    parser.add_argument("--pred-file", type=Path, default=DEFAULT_PREDICTION_FILE)
    parser.add_argument("--output-prefix", default=DEFAULT_OUTPUT_PREFIX)
    parser.add_argument("--model-label", default="Moirai 1.1 Seq2Seq")
...
def main() -> None:
    ...
    if not pred_file.exists():
        print(f"Error: {pred_file} not found. Run 09_moirai1_seq2seq_inference.py first.")
        sys.exit(1)
    ...
    print("\n" + "=" * 60)
    print(f"  {args.model_label.upper()} — CLUSTER: {CLUSTER_NAME}")
    print("  METRIC VIEWS")
    print("=" * 60)
    ...
    all_metrics = {
        "pipeline": "phase3_direct_ghi_moirai1_seq2seq",
    ...
    metrics_path = RESULTS_DIR / f"{prefix}_evaluation_metrics.json"
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump(all_metrics, f, indent=2, default=str)

    report_lines = [
        f"{args.model_label.upper()} EVALUATION — Cluster: {CLUSTER_NAME}",
        f"Prediction file: {pred_file.name}",
    ...
    report_path = RESULTS_DIR / f"{prefix}_evaluation_report.txt"
    ...
    try:
        plot_two_week_comparison(
            ...
            title=f"{args.model_label} — One-Week Measured vs Predicted ({plot_station_id})",
            save_path=str(RESULTS_DIR / f"{prefix}_one_week.png"),
    ...
    try:
        plot_two_week_comparison(
            ...
            window_days=ONE_WEEK_DAYS,
            title=f"{args.model_label} — Random Later One-Week ({plot_station_id})",
            save_path=str(RESULTS_DIR / f"{prefix}_random_week.png"),
    ...
    try:
        plot_4panel_evaluation(
            ...
            title=f"{args.model_label} — Cluster Evaluation ({CLUSTER_NAME})",
            save_path=str(RESULTS_DIR / f"{prefix}_4panel.png"),
    ...
    for sid in station_metrics:
        try:
            ...
            plot_4panel_evaluation(
                ...
                title=f"{args.model_label} — {sid}",
                save_path=str(RESULTS_DIR / f"{prefix}_4panel_{sid}.png"),
                n_points=len(sdf),
            )
        except Exception as e:
            print(f"  Warning: Could not generate 4-panel for {sid}: {e}")

    report_cols = ["station_id", "datetime", "hour", "lead_time_h", "GHI_true", "GHI_pred"]
    ...
    validation_csv_path = RESULTS_DIR / f"{prefix}_validation_report_data.csv"
    validation_report.to_csv(validation_csv_path, index=False)
