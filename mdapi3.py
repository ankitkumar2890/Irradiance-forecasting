...
def preprocess_json(raw_json):
    """
    Escapes Unescaped Backslashes for Windows-style Paths provided in 'config3.json' 
    """
...
def load_config(): 
    """Loads and validates configuration from config3.json."""
    try:
        ...
        try:
            ...
            try:
                ...
                print("[ERROR] Invalid JSON format in 'config3.json'! Please correct it and Try Again.")
                sys.exit(1)
        ...
        for field in required_fields:
            if field not in config:
                raise ValueError(f"Missing Required Config Section: {field} inside 'config3.json'")
            
        # Sets Current Directory for Download if 'download_settings' not specified
        if "download_settings" not in config:
            print("\n[Warning]: 'download_settings' not set in 'config3.json'. Downloading in the Current Directory..")
        ...
        print("[ERROR] 'config3.json' Not Found!")
        exit(1)
        
    except ValueError as e:
        print(f"[ERROR] in 'config3.json': {e}")
        exit(1)

config_file = load_config()

# Fetching Information from 'config3.json'
...
# Retrives Download Path from 'config3.json'
...
# Validates Fields of 'download_settings' of 'config3.json'
...
if invalid_fields:
    ...
    print("\nPlease Correct these in your 'config3.json' and Try Again.\n")
    sys.exit(1)
...
def search_results():
    ...
    try:
        res = requests.get(search_url, params=data, timeout=30)
        if res.status_code == 200:
            ...
            print(f"\n[ERROR] Error Fetching Data from Search Endpoint. Please enter correct 'search_parameters' in your 'config3.json' and try again.\n\nStatus Code: {res.status_code}\nError Message: {error_message}\n")
            if generate_logs:
...
def fetch_and_download_data(total_files, access_token, refresh_token):
    ...
    try:
        res = requests.get(check_internet_url, json=data, timeout=30)
        if res.status_code == 200:
            list = res.json()
            if list[0][0] == 1:
...
def get_user_input():
    try:
        if skip_user_input:
            print(f"\n{GREEN}'skip_user_input' Option Enabled in 'config3.json'. Proceeding with Download...{RESET}")
            return "yes"
    ...
    while True:
        ...
        for attempt, delay in enumerate(RETRY_DELAYS + [None]):
            try:
                ...
                response = requests.get(download_url, headers=headers, params=params, stream=True, timeout=30)
...
def logout():
    ...
    for attempt, delay in enumerate(retry_delays):
        try:
            response = requests.post(logout_url, json=data, timeout=30)
