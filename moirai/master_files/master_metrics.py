...
def calculate_metrics(actual, predicted, mape_threshold=5.0):
    ...
    nrmse = (rmse / mean_actual) * 100 if mean_actual > 0 else float("nan")

    mape_mask = actual >= mape_threshold
    if np.sum(mape_mask) > 0:
        denom_floor = max(float(mape_threshold), 1e-8)
        denom = np.maximum(actual[mape_mask], denom_floor)
        mape = np.mean(np.abs((predicted[mape_mask] - actual[mape_mask]) / denom)) * 100
    else:
        mape = 0
