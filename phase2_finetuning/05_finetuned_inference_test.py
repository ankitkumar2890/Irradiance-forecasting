"""Run fine-tuned Moirai (base + LoRA adapter) on the test split."""
import sys, json
import torch
import numpy as np
import pandas as pd
...
def main():
    ...
    station_ids = np.load(DATASET_DIR / "station_ids_test.npy", allow_pickle=True)
    ...
    def forecast_mean(target, feat_dynamic_real):
        past_target = torch.tensor(target, dtype=torch.float32, device=device).view(1, CONTEXT_LENGTH, TARGET_DIM)
        ...
        feat_dynamic = torch.tensor(feat_dynamic_real.T, dtype=torch.float32, device=device).unsqueeze(0)
    ...
    pred_path = RESULTS_DIR / "finetuned_predictions_test.csv"
    ...
    metrics_path = RESULTS_DIR / "finetuned_metrics_test.json"
