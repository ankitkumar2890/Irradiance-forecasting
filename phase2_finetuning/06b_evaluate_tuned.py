"""Evaluate grid-searched fine-tuned predictions: finds best quantile, generates metrics & plots."""
import sys, json
import numpy as np
import pandas as pd
import matplotlib
...
def main():
    # ---- Load gridsearch predictions ----
    pred_file = RESULTS_DIR / "finetuned_predictions_tuned_gridsearch.csv"
    if not pred_file.exists():
        print(f"Error: {pred_file} not found. Run 05b_finetuned_inference_tuned.py first.")
        sys.exit(1)

    df = pd.read_csv(pred_file)
    df["datetime"] = pd.to_datetime(df["datetime"])

    # Extract all the dynamically searched quantiles
    pred_cols = [c for c in df.columns if c.startswith("CAF_pred_")]
    quantiles = [float(c.split("_")[-1]) for c in pred_cols]
    ...
    print("\n" + "=" * 55)
    print("  GRID SEARCH RESULTS (Daytime CAF RMSE)")
    print("=" * 55)
    
    best_q = None
    best_rmse = float("inf")
    
    for q_col in pred_cols:
        r = np.sqrt(np.mean((daytime[q_col] - daytime["CAF_true"]) ** 2))
        print(f"    {q_col:15s}: RMSE = {r:.4f}")
        if r < best_rmse:
            best_rmse = r
            best_q = q_col
            
    print(f"\n  >> BEST QUANTILE: {best_q} (RMSE: {best_rmse:.4f}) <<")
    
    # Establish the chosen best predictions as the primary "CAF_pred"
    df["CAF_pred"] = df[best_q]
    daytime["CAF_pred"] = daytime[best_q]
    best_q_num = best_q.split("_")[-1]

    # ---- GHI recovery ----
    df["GHI_true"] = df["CAF_true"] * df["clear_sky_ghi"]
    df["GHI_pred"] = df["CAF_pred"] * df["clear_sky_ghi"]
    daytime["GHI_true"] = df.loc[daytime.index, "GHI_true"]
    daytime["GHI_pred"] = df.loc[daytime.index, "GHI_pred"]
    ...
    # ---- CAF Metrics (daytime) ----
    caf_m = compute_metrics(daytime["CAF_true"].values, daytime["CAF_pred"].values, f"CAF_tuned_{best_q_num}")
    caf_m["Skill_vs_persist"] = 1.0 - caf_m["RMSE"] / persist_rmse if persist_rmse > 0 else float("nan")

    # ---- GHI Metrics (daytime) ----
    ghi_m = compute_metrics(daytime["GHI_true"].values, daytime["GHI_pred"].values, f"GHI_tuned_{best_q_num}")

    print("\n" + "=" * 55)
    print(f"  TUNED MODEL ({best_q_num} Quantile) — SUMMARY")
    print("=" * 55)
    ...
    # ---- Save all metrics ----
    all_metrics = {
        "best_quantile": best_q_num,
        "caf": caf_m, 
        "ghi": ghi_m, 
        "persistence_rmse": float(persist_rmse)
    }
    with open(RESULTS_DIR / "evaluation_metrics_tuned_gridsearch.json", "w") as f:
    ...
    fig.suptitle(f"Fine-Tuned Moirai — BEST TUNED ({best_q_num} Quantile) - Nov 2019", fontsize=13, fontweight="bold")

    ax = axes[0]
    ax.plot(week["datetime"], week["GHI_true"], label="GHI True", color="#1f77b4", lw=1.5)
    ax.plot(week["datetime"], week["GHI_pred"], label=f"GHI Pred ({best_q_num} quantile)", color="#2ca02c", lw=1.3, ls="--")
    ...
    ax2.plot(week["datetime"], week["CAF_pred"], label=f"CAF Pred ({best_q_num} quantile)", color="#2ca02c", lw=1.3, ls="--")
    ...
    plt.tight_layout()
    fig.savefig(RESULTS_DIR / "finetuned_timeseries_tuned.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    ax3.set_title(f"Fine-Tuned Scatter ({best_q_num} Quantile) | RMSE={caf_m['RMSE']:.4f}")
    fig2.savefig(RESULTS_DIR / "finetuned_scatter_tuned.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    if horizon_rmse:
        ...
        ax4.plot([h+1 for h in hours], rmses, "o-", color="#2ca02c")
        ax4.set_xlabel("Forecast Hour")
        ax4.set_ylabel("CAF RMSE")
        ax4.set_title(f"Forecast Degradation Curve ({best_q_num} Quantile)")
        ax4.grid(True, alpha=0.3)
        fig3.savefig(RESULTS_DIR / "finetuned_horizon_rmse_tuned.png", dpi=150, bbox_inches="tight")
        plt.close()

    print(f"\n  Plots saved → {RESULTS_DIR}/\n     - finetuned_timeseries_tuned.png\n     - finetuned_scatter_tuned.png\n     - finetuned_horizon_rmse_tuned.png")
    print("Done.")
