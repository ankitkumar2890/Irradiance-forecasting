"""LoRA fine-tuning of Moirai 1.1 on CAF data using direct ERA5 covariates."""
import argparse
import sys, json
import numpy as np
import pandas as pd
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Moirai on direct-ERA5 CAF data.")
    parser.add_argument(
        "--smoke-test",
        action="store_true",
    ...
    def npy_to_gluonts(split):
        ...
        for i in range(len(X_past)):
            ...
            # Covariates are all non-target ERA5 and calendar features.
