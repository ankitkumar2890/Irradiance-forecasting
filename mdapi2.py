...
def preprocess_json(raw_json):
    """
    Escapes Unescaped Backslashes for Windows-style Paths provided in 'config2.json' 
    """
...
def load_config(): 
    """Loads and validates configuration from config2.json."""
    try:
        ...
        try:
            ...
            try:
                ...
                print("[ERROR] Invalid JSON format in 'config2.json'! Please correct it and Try Again.")
                sys.exit(1)
        ...
        for field in required_fields:
            if field not in config:
                raise ValueError(f"Missing Required Config Section: {field} inside 'config2.json'")
            
        # Sets Current Directory for Download if 'download_settings' not specified
        if "download_settings" not in config:
            print("\n[Warning]: 'download_settings' not set in 'config2.json'. Downloading in the Current Directory..")
        ...
        print("[ERROR] 'config2.json' Not Found!")
        exit(1)
        
    except ValueError as e:
        print(f"[ERROR] in 'config2.json': {e}")
        exit(1)

config_file = load_config()

# Fetching Information from 'config2.json'
...
# Retrives Download Path from 'config2.json'
...
# Validates Fields of 'download_settings' of 'config2.json'
...
if invalid_fields:
    ...
    print("\nPlease Correct these in your 'config2.json' and Try Again.\n")
    sys.exit(1)
...
def search_results():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"\n[ERROR] Error Fetching Data from Search Endpoint. Please enter correct 'search_parameters' in your 'config2.json' and try again.\n\nStatus Code: {res.status_code}\nError Message: {error_message}\n")
            if generate_logs:
...
def get_user_input():
    try:
        if skip_user_input:
            print(f"\n{GREEN}'skip_user_input' Option Enabled in 'config2.json'. Proceeding with Download...{RESET}")
            return "yes"
